...
int main(int argc, char *argv[]) {
    ...
    #if defined(_POSIX_ADVISE) && _POSIX_ADVISE > 0
    int fadvise_result = posix_fadvise(fd, 0, 0, POSIX_FADV_SEQUENTIAL);
    
    if (fadvise_result != 0) {
        fprintf(stderr, "Warning: fadvise failed (error %d: %s)\n", 
                fadvise_result, strerror(fadvise_result));
    } else {
        fprintf(stderr, "Applied fadvise: POSIX_FADV_SEQUENTIAL\n");
    }
    #else
    fprintf(stderr, "Warning: fadvise not supported on this system\n");
    #endif
